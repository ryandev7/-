...
<body>
	...
	String email = (loginUser.getEmail() == null) ? "" : m.getEmail();
	String address = (loginUser.getAddress() == null) ? "" : m.getAddress();
	String phone = (loginUser.getPhone() == null) ? "" : m.getPhone();
	String nickName = (loginUser.getNickName() == null) ? "" : m.getNickName();
	String cookLevel = (loginUser.getCookLevel() == null) ? "" : m.getCookLevel();
	String favoriteFood = (loginUser.getFavoriteFood() == null) ? "" : m.getFavoriteFood();
	...
	<div class="menu">
	 ...
	 <div class="outer">
        	...
        	<hr>
			<br>
			<form id="update-form" action="<%= contextPath %>/AdUpdate.me" method="post">
			...
			<script>
				...
				$('input[type=radio]').each(function(){
					...
					if(favoriteFood.search($(this).val()) != -1){ 
						$(this).attr("checked", true);
					}
				})
	...
	<div class="modal" id="updatePwdForm">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form action="<%= contextPath %>/AdupdatePwd.me" method="post">
					...
					<table>
						<tr>
							<td>현재 비밀번호</td>
							<!-- 관리자는 비밀번호 그냥 맘대로 바꿔도 됨 기본으로 입력되어있게끔 -->
							<td><input type="password" required name="userPwd" value=<%=userPwd %>></td>
						</tr>
						<tr>
							<td>변경할 비밀번호</td>
