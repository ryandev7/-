	...
	<div class="outer">
		...
		<table class="list-area">
			...
			<tbody>
				<% if(list.isEmpty()) { %> <!-- 조회글 없음 -->
					<tr>
						<td colspan="4">조회된 나의 리뷰가 없습니다.</td>					
					</tr>
				<% } else {%>
		...
		<script>
			...
			$(function(){
				...
				$(".list-table>tbody>tr").click(function(){
					...
					location.href="<%= contextPath %>/detail.pr?bno=" + bno;
				})
		...
		<div align="center" class="paging-area">
			...
			<% if(currentPage != 1) {%> <!-- 페이징바에서 <를 담당 -->
				<button class="btn btn-sm btn-info" onclick="location.href='<%= contextPath %>/myReview.me?userId=<%=loginUser.getUserId() %>&cpage=<%= currentPage - 1 %>'"> &lt; </button>
			<% } %>

			<% for(int i = startPage; i <= endPage; i++) { %>
				<% if(i != currentPage) {%>
					<button class="btn btn-sm btn-info" onclick="location.href='<%= contextPath %>/myReview.me?userId=<%=loginUser.getUserId() %>&cpage=<%= i %>'"><%= i %></button>
				<% } else { %>
