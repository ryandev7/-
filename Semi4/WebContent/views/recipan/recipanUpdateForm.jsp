...
<body>
	...
	<div id="area">
        <form action="<%=contextPath%>/update.pan" method="post" enctype="multipart/form-data">
        <input type="hidden" name = "peNo" value ="<%=recipan.getPeNo()%>">
        <div id ="recipanContent1">
            ...
            <div id="recipanForm1">                
                <table id="recipanContent1Table">
                    <tr>
                        <th>레시판 제목</th>
                        <td><input type="text" name="recipanTitle" id="" placeholder="레시피 제목을 입력하세요" value="<%=recipan.getPeTitle() %>"></td>
                    </tr>
                    <tr>
                        <th>요리 소개</th>
                        <td><input type="text" name="recipanIntro" id="" placeholder="레시피 설명을 입력하세요" value="<%=recipan.getPeIntroduce()%>"></td>
                    </tr>
                    <tr>
                        <th>카테고리</th>
                        <td>
                        	<%if(!categoryList.get(2).isEmpty()){ %>
                            <select name="nationalCat" id="natCat">
                            ...
                            <select name="kindCat" id="kindCat">
                            ...
                            <select name="productCat" id="proCat">
                    ...
                    <tr>
                        <th>요리정보</th>
                        <td>
                            <input type="text" name="recipanPerson" id="" placeholder="인원(명수까지)" value="<%=recipan.getPeFoodAmount()%>" required>
                            <input type="text" name="recipanTime" id="" placeholder="시간(단위까지)" value="<%=recipan.getPeCookTime() %>" required>
                            <input type="text" name="recipanDifficulty" id="" placeholder="난이도(상/중/하)" value="<%=recipan.getPeCookLevel()%>"required>
                        </td>
                    </tr>
                </table>
            </div>
            <div id="recipanForm2">
                <div style="border : 1px solid gray; width : 180px; height : 200px;"><img src="<%=recipan.getTitleImg() %>" width="100%"height="100%"alt="" id="titleImg"></div>
        ...
        <div id = "recipanContent2">
            ...
            <div id="recipanProductForm">
                ...
                <table id="productTable">
                	<%for(RecipanProSau p : product){ %>
	                    <tr>                        
	                        <td><input type="text" name="" class="productName" placeholder="재료명" value="<%=p.getOutputName()%>" required></td>
                            <td><input type="text" name="" class="productWeight" placeholder="재료양(단위까지 입력)" value="<%=p.getOutputAmount()%>"required></td>
            ...
            <div id="recipanSauceForm">
                ...
                <table id="sauceTable">
                <%for(RecipanProSau s : sauce){ %>
	                    <tr>                        
	                        <td><input type="text" name="" class="sauceName" placeholder="소스명" value="<%=s.getOutputName()%>"required></td>
                            <td><input type="text" name="" class="sauceWeight" placeholder="소스양(단위까지 입력)" value="<%=s.getOutputAmount()%>"required></td>
        ...
        <div id="recipanContent3">
            ...
            <div id="recipanCookStep">
                ...
                <%for(RecipanCookStep c : cookStep){ %>
                    <tr>
                        <th><div class = "imgDiv"><img src="<%=c.getStepImg() %>" alt="cookStep" width="100%" height="100%" class="cookStepImgTag"></div></th>
                        <td><input type="file" hidden name="" class="cookStepFile"  onchange="cookStepImg(this);"></td>
                        <td><textarea name="" class="cookStepIntro" cols="45" rows="10"style="resize: none;"><%=c.getOutPutStepContent() %></textarea></td>
                ...
                <div id="addToCookStepForm">
                    <button type="button" id="addRecipanCookStepBtn" class="btn btn-secondary">+</button>
                    <br><br>
                    <button type="button" id="indexGiveNamebtn" class="btn btn-secondary">수정 완료 !</button>
                    <input type="submit" id="submitRecipan" hidden>
                </div>
                           
                <button type="button" id="modalBtn" class="btn btn-primary" data-toggle="modal" data-target="#staticBackdrop" style="visibility: hidden;">
                </button>
            </div>
        </div>
        ...
        <script>
            ...
            function cookStepImg(inputFile){
                ...
                if(inputFile.files.length == 1){
                    ...
                    reader.onload = function(e){
                        ...
                        // $(inputFile).parent().find("img").attr("src",e.target.result);                        
                    }
                }else{
            ...
            $(document).on("click", "#indexGiveNamebtn", function(){
                        ...
                        $(".cookStepFile").each(function(index, item){
                            ...
                            console.log("cookStepFile" + index);
            ...
            $(function(){
            	$("#natCat").val(<%=recipan.getNatCatNo()%>).prop("selected",true);            
            	$("#kindCat").val(<%=recipan.getKindCatNo()%>).prop("selected",true);
            	$("#proCat").val(<%=recipan.getProCatNo()%>).prop("selected",true);
                ...
                $("#addProductTableBtn").on("click",function(){
                    $("#productTable").append('<tr><td><input type="text" name="" class="productName"></td><td><input type="text" name="" class="productWeight"></td><td><button type="button" class="removeProductBtn btn btn-secondary">-</button></td></tr>');
                })   
                $("#addSauceTableBtn").on("click",function(){
                    $("#sauceTable").append('<tr><td><input type="text" name="" class="sauceName"></td><td><input type="text" name="" class="sauceWeight"></td><td><button type="button" class="rmoveSauceBtn btn btn-secondary">-</button></td></tr>');
                })     
