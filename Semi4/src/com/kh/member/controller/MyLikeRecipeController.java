...
/**
 * Servlet implementation class MyLikeRecipe
 */
@WebServlet("/myLikeRecipe.me")
public class MyLikeRecipeController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MyLikeRecipeController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new MemberService().selectRecipanCount(userId);
		pageLimit = 10;
		...
		ArrayList<Recipan> list = new MemberService().selectRecipan(userId, pi);
		
		request.setAttribute("list", list); // 우리가 실제로 조회한 페이지에 보일 10개의 게시글
		request.setAttribute("pi", pi); // 페이징바를 만들 때 필요한 변수

		request.getRequestDispatcher("views/member/myLikeRecipe.jsp").forward(request, response);
